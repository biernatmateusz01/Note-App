        ...
        <BaseModal @close-modal="closeModal" v-if="isOpen">
          <form @submit.prevent="addNewNote">
            ...
            >
              <span class="mb-2"
                >Add new note for
                <span class="text-blue-400 font-semibold">Tuesday</span></span
              >
...
const getNotesFromStorage = () => {
  const booksFromStorage = localStorage.getItem("tuesdayNotes");
...
onMounted(() => {
  if (localStorage.hasOwnProperty("tuesdayNotes")) {
  } else {
    localStorage.setItem("tuesdayNotes", JSON.stringify(notes.value));
  }
  getNotesFromStorage();
