        ...
        <BaseModal @close-modal="closeModal" v-if="isOpen">
          <form @submit.prevent="addNewNote">
            ...
            >
              <span class="mb-2"
                >Add new note for
                <span class="text-blue-400 font-semibold">Thursday</span></span
              >
...
const deleteAll = () => {
  notes.value = [];
  localStorage.setItem("thursdayNotes", JSON.stringify(notes.value));
...
const getNotesFromStorage = () => {
  const booksFromStorage = localStorage.getItem("thursdayNotes");
...
onMounted(() => {
  if (localStorage.hasOwnProperty("thursdayNotes")) {
  } else {
    localStorage.setItem("thursdayNotes", JSON.stringify(notes.value));
  }
  getNotesFromStorage();
